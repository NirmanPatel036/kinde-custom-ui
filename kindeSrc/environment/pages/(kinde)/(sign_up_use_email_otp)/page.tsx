...
// This is a copy for sign_up flow, reuses same OTP layout
const SignUpEmailOtpPage: React.FC<KindePageEvent> = ({ context }) => {
    const heading = context?.widget?.content?.heading ?? "Verify your email";
        ...
        <>
            ...
            <form method="POST" action="">
                <div className="otp-inputs">
                    {[...Array(6)].map((_, i) => (
                        <input key={i} type="text" inputMode="numeric" maxLength={1} pattern="\d" name={`otp_${i}`} aria-label={`OTP digit ${i + 1}`} required />
                    ))}
                </div>
                <button className="auth-btn" type="submit">Complete sign up</button>
            </form>
...
export default async function Page(event: KindePageEvent): Promise<string> {
    const page = await SignUpEmailOtpPage(event);
